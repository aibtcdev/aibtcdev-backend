...
def pretty_print_result(result: ComprehensiveEvaluatorAgentProcessOutput):
    ...
    if result.categories:
        ...
        for category in result.categories:
            print(f"  • {category.category}: {category.score}/100 (Weight: {category.weight:.1%})")
...
async def test_evaluation(proposal_id: str, model: str = None, save_output: bool = False):
    ...
    try:
        ...
        proposal_content = getattr(proposal, 'content', '') or getattr(proposal, 'summary', '') or proposal.title
        ...
        if hasattr(proposal, 'tweet_id') and proposal.tweet_id:
            try:
                from app.services.ai.simple_workflows.processors.twitter import fetch_tweet, format_tweet, format_tweet_images
                ...
                if tweet_data:
                    ...
                    print(f"📝 Retrieved tweet content: {len(tweet_content)} characters")
                    ...
                    for i, img in enumerate(linked_tweet_images):
                        print(f"  Image {i+1}: {img.get('image_url', {}).get('url', 'No URL')}")
                else:
        ...
        if hasattr(proposal, 'airdrop_id') and proposal.airdrop_id:
            try:
                ...
                airdrop_content = await process_airdrop(proposal.airdrop_id, proposal_uuid)
                if airdrop_content:
                    print(f"🪂 Retrieved airdrop content: {len(airdrop_content)} characters")
                else:
...
"""
        ...
        past_proposals = "<no_proposals>No past proposals available for comparison.</no_proposals>"
        if hasattr(proposal, 'dao_id') and proposal.dao_id:
            try:
                from app.services.ai.simple_workflows.evaluation import fetch_dao_proposals, format_proposals_for_context_v2
        ...
        from app.services.ai.simple_workflows.evaluation_openrouter_v1 import create_chat_messages
        ...
        system_message = next((msg for msg in full_messages if msg["role"] == "system"), {})
        ...
        token_usage = {
            ...
            "output_tokens": len(raw_response) // 4 if raw_response != "Not available" else 0,
            "total_tokens": (len(str(full_messages_dict)) + len(raw_response)) // 4 if raw_response != "Not available" else len(str(full_messages_dict)) // 4
        }
        ...
        result_dict = {
            ...
            "full_system_prompt": system_message.get("content", "No system prompt found"),
        ...
        if save_output:
            ...
            # Save JSON summary
            json_filename = f"evals/{timestamp}_prop01_{prop_short_id}_summary_openrouter.json"
